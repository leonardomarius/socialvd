...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (profile && profile.pseudo && profile.bio) {
        router.replace("/feed");
        return;
      }
  ...
  const handleComplete = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (passwordError) {
        setErrorMsg("Unable to set password: " + passwordError.message);
        setLoading(false);
        return;
      }

      // 2) Mettre à jour le profil (pseudo et mindset)
    ...
    >
      ...
      >
        ...
        <p style={{ marginBottom: 24, fontSize: "0.875rem", color: "rgba(255, 255, 255, 0.7)", textAlign: "center" }}>
          Please complete your SocialVD account by choosing a username, password, and mindset.
        </p>
